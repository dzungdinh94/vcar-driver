...
export default class NavBarCustom extends Component {
    render() {
        let { onPress, title, leftButton, colorBtn, backgroundColor } = this.props;
        return (
            <View style={[{
                height: values.toolbarHeight, width: '100%',
                justifyContent: 'flex-end', alignItems: 'center',
            }, backgroundColor ? { backgroundColor: backgroundColor } : { backgroundColor: color.primaryColor }]
            }>
                <View style={[{ width: '100%', flexDirection: 'row', alignItems: 'center', },
                values.platform === 'android' ? { height: values.toolbarHeight } : { height: 44, }
                ]}>
